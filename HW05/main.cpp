...
void initialise()
{
    ...
    g_display_window = SDL_CreateWindow("MEOW KNIGHT",
...
void update()
{
    ...
    if (g_current_scene == g_level_a && g_current_scene->get_state().player->get_position().x >= 16.0f) switch_to_scene(g_level_b);

    if (g_current_scene == g_level_b && g_current_scene->get_state().player->get_position().x >= 16.0f && g_current_scene->get_state().player->get_position().y <= 5.5f) switch_to_scene(g_level_c);
...
void render()
{
    ...
    if (((g_current_scene == g_level_c) && g_current_scene->get_state().player->get_position().x >= 16.0f) || g_current_scene->get_state().player->all_levels_passed)
    {
        // Y
        Utility::draw_text(&g_shader_program, font_texture_id, "Y", 0.5f, 0.05f,
            glm::vec3(g_current_scene->get_state().player->get_position().x - 1.5f, -3.0f, 0.0f));

       // O
            Utility::draw_text(&g_shader_program, font_texture_id, "O", 0.5f, 0.05f,
                glm::vec3(g_current_scene->get_state().player->get_position().x - 1.0f, -3.0f, 0.0f));

       // U
            Utility::draw_text(&g_shader_program, font_texture_id, "U", 0.5f, 0.05f,
                glm::vec3(g_current_scene->get_state().player->get_position().x - 0.5f, -3.0f, 0.0f));
